...
import { BirthdayPaywall } from '@/components/contacts/BirthdayPaywall';
...
export function BirthdaysPage() {
  ...
  // Fail closed: until we actually know the entitlement, don't render the real
  // (unblurred, interactive) page - that's what let the page be briefly usable
  // on reload before the paywall query resolved.
  const birthdayUnlocked = entitlements.data?.birthdayUnlocked ?? false;
  const birthdayAddonGhs = entitlements.data?.addons.find((a) => a.key === 'birthday_automation')?.ghs ?? 0;
