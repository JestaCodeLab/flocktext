...
import { extendAdminSession, adminLogout } from '@/api/adminAuth';
...
export function AdminSessionTimeoutModal() {
  ...
  // api/services/adminSession.js), only on the explicit "stay signed in" call
  // below. Mirrors client/src/components/layout/SessionTimeoutModal.tsx.
  useEffect(() => {
    setSecondsLeft(null);
    setDismissed(false);
  ...
  async function handleStay() {
    ...
    setStaying(true);
    try {
      const data = await extendAdminSession(refreshToken);
