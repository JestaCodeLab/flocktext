        ...
        {steps.map((step, i) => {
          ...
          return (
            <Fragment key={step.key}>
              <div className="flex w-24 shrink-0 flex-col items-center text-center sm:w-32">
                <span
                  className={cn(
              ...
              {i < steps.length - 1 && (
                <div className={cn('mt-5 h-0.5 flex-1 rounded-full transition-colors', lineActive ? 'bg-primary/40' : 'bg-border')} />
              )}
            </Fragment>
          );
        })}
